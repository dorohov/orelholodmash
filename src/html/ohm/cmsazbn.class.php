...
public $__loadrow = array();
	...
	*/
	...
	public function as_phone($str)
	{
		return preg_replace('/[^0-9]/', '', $str);
	}
	
	public function rebuildNewsArchive()
	{
		$news_archive = array();
		
		$item_list = $this->FE->DB->dbSelect("SELECT * FROM `".$this->FE->DB->dbtables['t_news']."` WHERE visible='1' AND (1) ORDER BY date");
		if(mysql_num_rows($item_list)) {
			while($row = mysql_fetch_array($item_list)) {
				$news_archive[date('Y', $row['date'])][date('m', $row['date'])]++;
			}
			mysql_data_seek($item_list,0);
		}
		
		$this->setParamValue('news_archive', serialize($news_archive));
	}
	
	public function str_month($m)
	{
		$m_arr = array(
			1 => 'Январь',
			2 => 'Февраль',

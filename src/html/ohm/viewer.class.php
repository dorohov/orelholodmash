...
public $class_name='viewer';
	...
	public function startofpage(&$param, $tpl='startofpage')
	{
		if($tpl!=''){
			...
			if($param['mdl']['page_header'] == '') {
				$param['mdl']['page_header'] = 'default/page_header';
			}
			
			if($param['mdl']['__header-site'] == '') {
				$param['mdl']['__header-site'] = 'default/header-site';
			}
	...
	*/
	...
	public function email_tpl($tpl,&$param)
	{
		$code = '';
		...
		if($tpl != ''){
			...
			$this->FE->Cache->start_caching();
			$file = 'sites/'.$this->FE->config['site'].'/tpl/'.$tpl.'.tpl.php';
			if(file_exists($file)) {
				require($file);
			}
			$code = $this->FE->Cache->get_caching_content();
			$this->FE->Cache->finish_caching();
